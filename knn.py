...
def main():
    ...
    for k in [1,3,5,7,9,99,999,8000]:
      ...
      predicted = predict(train_X, train_y, train_X, k)     #Predict the classes
      train_acc = compute_accuracy(train_y, predicted)      #Compute the accuracy of the prediction
      ...
      val_acc, val_acc_var = cross_validation(train_X, train_y, num_folds=4, k=k)   #Run cross validation
...
def get_nearest_neighbors(example_set, query, k):
    ...
    distances = []                                                              #Create array to hold all distances
    
    for item in example_set: distances.append(np.linalg.norm(item - query))     #Iterate through all points, computing the distance to the query point
    
    idx_of_nearest = np.argsort(distances)[:k]                                  #Set variable to the list of the k nearest items by distance
...
def knn_classify_point(examples_X, examples_y, query, k):
    ...
    nn = get_nearest_neighbors(examples_X, query, k)            #Get nearest neighbors
    nnClass = examples_y[nn]                                    #Get nn classes
    classTotals = {}
    
    for i in nnClass:                                           #Iterate through the classes, counting the occurancies of each
        if i[0] not in classTotals: classTotals[i[0]] = 1
        else: classTotals[i[0]] += 1
        
    for i, j in classTotals.items():                            #Return the most class with the most occurances
        if j == max(classTotals.values()): return i 
...
def cross_validation(train_X, train_y, num_folds=4, k=1):
    ...
    trainX = np.split(train_X, num_folds)                               #Split each matrix in num_folds ways
    trainY = np.split(train_y, num_folds)
    ...
    for i in range(num_folds):
        ...
        trainX_concat = np.concatenate(trainX[: i] + trainX[i + 1 :])
        trainY_concat = np.concatenate(trainY[: i] + trainY[i + 1 :])
        
        predicted = predict(trainX_concat, trainY_concat, trainX[i], k) #Run the prediction for the test

        predictedAccuracy = compute_accuracy(trainY[i], predicted)      #Compute the accuracy
        accuracies.append(predictedAccuracy)                            #Append to a list
        #End loop    

    avg_val_acc = sum(accuracies) / num_folds                           #Calculate the mean
    varr_val_acc = avg_val_acc * (1 - avg_val_acc) / len(train_X)       #Calculate the variance
